...
HX_STACK_PUSH("AppMain::new","com/appframework/core/AppMain.hx",13);
{
	...
	HX_STACK_LINE(33)
	::nme::Lib_obj::get_current()->get_stage()->addEventListener(::native::events::Event_obj::RESIZE,this->onAddedToStage_dyn(),null(),null(),null());
}
;
	return null();
}
...
Void AppMain_obj::onAddedToStage( ::native::events::Event event){
{
		...
		HX_STACK_ARG(event,"event");
		HX_STACK_LINE(48)
		::nme::Lib_obj::get_current()->get_stage()->removeEventListener(::native::events::Event_obj::RESIZE,this->onAddedToStage_dyn(),null());
		HX_STACK_LINE(53)
		this->startApp();
	}
return null();
